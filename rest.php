<?php
    ...
    class Rest {
        ...
        public function __construct(){
            ...
            $db = new DbConnect; // database connection
        ...
        public function validateRequest() {  
            if($_SERVER['CONTENT_TYPE'] !== 'application/json') {  // check if this is json content
				$this->throwError(REQUEST_CONTENTTYPE_NOT_VALID, 'Request content type is not valid');
			}
			$data = json_decode($this->request, true);
            if(!isset($data['name']) || $data['name'] == "") {  // check if servicename wich is called is set
                $this->throwError(API_NAME_REQUIRED, 'API name is required.');
            } 
            $this->serviceName = $data['name'];

            if(!is_array($data['param'])) {  // check if array of parameters is set
                $this->throwError(API_PARAM_REQUIRED, 'API param is required.');
            } 
            $this->param = $data['param'];
        }

        public function processApi() {  // if method dosen t exist throw error
            $api = new API;
				$rMethod = new reflectionMethod('API', $this->serviceName);
				if(!method_exists($api, $this->serviceName)) {
					$this->throwError(API_DOST_NOT_EXIST, "API does not exist.");
                }
                $rMethod->invoke($api); // if exists call method
        }

        public function validateParameter($fieldName, $value, $dataType, $required = true){
            if($required == true && empty($value) == true) {  // if email and value is empty thow error
